...
public class Sampling {
	 ...
	 * 
	 * @param targetAreaResolution the target area resolution (how many rays on each
	 *                             column and row)
	 */
	 ...
	 * 
	 * @param targetAreaDistance the target area distance from the beginning of the
	 *                           ray
	 */
	...
	/**
	 * Generates a ray, starting at the point and going through specific square in
	 * grid.
	 *
	 ...
	 */
	private static Vector createVectorBeam(int i, int j, Ray ray, Vector vTo, Vector vUp, Vector vRight, double k,
			Vector n) {
		Point p0 = ray.getHead();
		...
		double yI = -(i - ((double) (TARGET_AREA_RESOLUTION - 1)) / 2) * rC; // vertical distance of the required square
																				// from the center of the grid
		double xJ = -(j - ((double) (TARGET_AREA_RESOLUTION - 1)) / 2) * rC; // horizontal distance of the required
																				// square from the center of the grid
		// changing the position of the center point so that the ray will intersect the
		// view plane in the right place
		if (xJ != 0)
			pIj = pIj.add(vRight.scale(xJ));
		if (yI != 0)
			pIj = pIj.add(vUp.scale(yI));
		// return the ray
		double sign = pIj.subtract(ray.getHead()).dotProduct(n);
		if (vTo.dotProduct(n) * sign < 0)
			return null; // Checking that the secondary ray doesn't go the other side of the normal plane
