...
/**
 * (Extends) class for ray tracing - calculates color of pixels use of Voxels
 *
...
//        return currentPixelColor;
	...
	/**
	 * calcColor - recursive function calculate local effects (diffuse, specular)
	 * and global effects (reflection, refraction)
	 *
	...
	/**
	 * calcColor - wrapper function calculate the color of point
	 *
	 * @param geoPoint geometry point to calculate its color
	 * @param ray      the ray that intersect geoPoint
	 * @return color of point, including all effects
...
//    }
	...
	private Color calcGlobalEffects(Intersectable.GeoPoint gp, Ray ray, int level, Double3 k) {
		...
		if (material.kDg != 0 && material.kSg != 0) {
			return glossSamplingSum.add(diffSamplingSum);
		}
	...
	/**
	 * calculate the global effects of specific level on color if there are more
	 * intersections to check
	 *
	 * @param ray   the is used to intersect the geometries
	 * @param level the current level
	 * @param k     a color factor to reduce the color (according to the current
	 *              level of recursion)
	...
	/**
	 * Implements the 3dda algorithm. It determines through which voxels the ray
	 * goes.
	 *
	...
	private Intersectable.GeoPoint traversalAlgorithm(Ray ray) {
		...
		double[] firstIntersectionCoordinates = new double[] { firstIntersection.xyz.d1, firstIntersection.xyz.d2,
				firstIntersection.xyz.d3 };
		...
		for (int i = 0; i <= 2; i++) {
			tMax[i] = determineTmax(boundary[i][0], steps[i], indexes[i], voxelEdges[i], directions[i],
					firstIntersectionCoordinates[i]);
		}
		...
		// second sub algo
		// move over all the geometries of the first voxel and find the closest
		...
		do {
			...
			if (list == null) {
				...
				if (farIntersection != null && isInsideVoxel(indexes, farIntersection.point, boundary)) {// if it's the
																											// voxel
																											// with the
																											// saved
																											// point
					...
					if (closestIntersection != null) {
						if (isInsideVoxel(indexes, closestIntersection.point, boundary)) {
							...
							if (closestIntersection.point
									.distanceSquared(fixedFirstIntersection) <= farIntersection.point
											.distanceSquared(fixedFirstIntersection)) {
								// even though we reached the voxel that contains the precalculated point, we
								// found closer one
								return closestIntersection;
							} else {
					...
					} else {// no other intersection in this voxel, therefore the saved intersection is the
							// closest
						return farIntersection;
					}
					...
					if (closestIntersection != null) {
						if (!isInsideVoxel(indexes, closestIntersection.point, boundary)) {
							// if not in the voxel
							if (farIntersection == null || closestIntersection.point
									.distanceSquared(fixedFirstIntersection) <= farIntersection.point
											.distanceSquared(fixedFirstIntersection)) {
								// if it's closer than the already saved farIntersection, save this, since tha
								// saved one would not intersect before this
								farIntersection = closestIntersection;
								closestIntersection = null;
							}
	...
	/**
	 * function that finds the geometric objects in all the voxels the ray travels
	 * through
	 *
	 * @param ray the ray through the scene voxels grid
	 ...
	 */
	// if there is no intersection points in the first voxel search in the rest of
	// the ray's way
	// since the ray starts in the middle of a voxel (since we moved it on the
	// intersection with the scene CBR,
	// or the head is already inside the scene SCR), we had to calculate the
	// remaining distance to the fist voxel's edge.
	// But from now on, we can use the constant voxel size, since it would always
	// intersect with the edge of the voxel.
	// now would do the same calculation on the rest of the ray's way in the voxels
	// grid.
	...
	private Point firstIntersection(Ray ray) {
		...
		if (x >= boundary[0][0] && x <= boundary[0][1] && y >= boundary[1][0] && y <= boundary[1][1]
	 ...
	 * @param intersectionCoordinate the first intersection of the ray with the
	 *                               regular grid
	 * @return the maximum step in units of t to the next voxel
	 */
	private double determineTmax(double minBoundary, double step, int index, double voxelEdge, double direction,
			double intersectionCoordinate) {
		if (step == 1) {
	...
	private boolean isInsideVoxel(int[] index, Point intersection, int[][] boundary) {
		...
		return intersection.xyz.d1 >= xMin && intersection.xyz.d1 <= xMax && intersection.xyz.d2 >= yMin
				&& intersection.xyz.d2 <= yMax && intersection.xyz.d3 >= zMin && intersection.xyz.d3 <= zMax;
	}

	/**
	 * fixes the intersection of the ray with the scene CBR on exact intersection
	 * with voxel edges
	 *
