...
class TriangleTests {
	...
	void findIntersections() {
		...
		assertNull(triangle.findIntsersections(new Ray(new Point(-1, 0, -1), new Vector(0, 0, 1))),
				"Wrong number of intersection points");

		// TC03: Ray outside the triangle against vertex (0)
		assertNull(triangle.findIntsersections(new Ray(new Point(3, 0, -1), new Vector(0, 0, 1))),
				"Wrong number of intersection points");

		// =============== Boundary Values Tests ==================

		// TC04: Ray start on a edge of triangle(0)
		assertNull(triangle.findIntsersections(new Ray(new Point(0, 0, -1), new Vector(0, 0, 1))),
				"Ray start on a edge of triangle");

		// TC05: Ray start on the extension of an edge of triangle(0)
		assertNull(triangle.findIntsersections(new Ray(new Point(2, 0, -1), new Vector(0, 0, 1))),
				"Ray start on the extension of an edge of triangle");

		// TC06: Ray start on a vertex of triangle
		assertNull(triangle.findIntsersections(new Ray(new Point(0, 3, -1), new Vector(0, 0, 1))),
